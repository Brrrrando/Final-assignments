...
# train stage one
def train(args):
    if os.path.exists(os.path.join(config.save_path, "stage1_loss.txt")):
        os.remove(os.path.join(config.save_path, "stage1_loss.txt"))
    ...
    train_data=torch.utils.data.DataLoader(train_dataset,batch_size=args.batch_size, shuffle=True, num_workers=0 , drop_last=True)
    ...
    for epoch in range(1,args.max_epoch+1):
        ...
        if epoch % 1==0:
            ...
            torch.save(model.state_dict(), os.path.join(config.save_path, 'model_stage1_epoch.pth'))    # 只会有一个权重，新的会覆盖旧的，不然很占空间


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train SimCLR')
    parser.add_argument('--batch_size', default=config.stage1_batchsize, type=int, help='')
