...
# train stage two
def train(args):
    if os.path.exists(os.path.join(config.save_path, "stage2_loss_top1-acc_top5-acc.txt")):
        os.remove(os.path.join(config.save_path, "stage2_loss_top1-acc_top5-acc.txt"))
    ...
    for epoch in range(1,args.max_epoch+1):
        ...
        if epoch % 1 == 0:
            torch.save(model.state_dict(), os.path.join(config.save_path, 'model_stage2_epoch.pth'))
            ...
            with torch.no_grad():
                ...
                with open(os.path.join(config.save_path, "stage2_loss_top1-acc_top5-acc.txt"), "a") as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', default=config.stage2_batchsize, type=int, help='')
